...
CREATE PROCEDURE gerar_dados_aleatorios()
BEGIN
    DECLARE i INT;

    -- Inserir 100 veterinários
    SET i = 1;
    WHILE i <= 100 DO
        INSERT INTO veterinarios (nome, especialidade)
        VALUES (CONCAT('Veterinário ', i), 
                CASE 
                    WHEN MOD(i, 3) = 0 THEN 'Cirurgião'
                    WHEN MOD(i, 3) = 1 THEN 'Dermatologista'
                    ELSE 'Cardiologista'
                END);
        SET i = i + 1;
    END WHILE;

    -- Inserir 1000 pacientes
    SET i = 1;
    WHILE i <= 1000 DO
        INSERT INTO pacientes (nome, especie, idade)
        VALUES (CONCAT('Paciente ', i), 
                CASE 
                    WHEN MOD(i, 3) = 0 THEN 'Cão'
                    WHEN MOD(i, 3) = 1 THEN 'Gato'
                    ELSE 'Pássaro'
                END,
                FLOOR(RAND() * 15) + 1); -- Idade entre 1 e 15
        SET i = i + 1;
    END WHILE;

    -- Inserir 10.000 consultas
    SET i = 1;
    WHILE i <= 10000 DO
        INSERT INTO consultas (id_paciente, id_veterinario, data_consulta, custo)
        VALUES (FLOOR(RAND() * 1000) + 1, -- ID aleatório de paciente
                FLOOR(RAND() * 100) + 1, -- ID aleatório de veterinário
                DATE_SUB(CURDATE(), INTERVAL FLOOR(RAND() * 365) DAY), -- Data aleatória no último ano
                ROUND(RAND() * 200, 2)); -- Custo aleatório entre 0 e 200
        SET i = i + 1;
    END WHILE;
